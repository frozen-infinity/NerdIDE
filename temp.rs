...
static BUFFERS: LazyLock<Mutex<Vec<Buffer>>> = LazyLock::new(|| {
    Mutex::new(Vec::new())
});
fn load_css() {
...
fn build_header(window: &ApplicationWindow, mut terminal: bool, notebook: Notebook) {
...
*/
    ...
    if terminal {
        ...
        paned.set_start_child(Some(&notebook4));
...
fn build_body(window: &ApplicationWindow, terminal: bool, notebook: Notebook) {
    let notebook2 = notebook.clone();
    build_header(&window, terminal, notebook2);
